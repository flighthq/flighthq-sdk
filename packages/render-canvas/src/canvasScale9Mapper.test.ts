import { createShapeData, drawRect } from '@flighthq/scenegraph-display';
...
describe('buildScale9Mapper', () => {
  it('returns null when scaleX is zero', () => {
    ...
    expect(buildScale9Mapper(data.commands, makeGrid(10, 10, 80, 80), 0, 1)).toBeNull();
  });

  it('returns null when scaleY is negative', () => {
    const data = createShapeData();
    drawRect(data, 0, 0, 100, 100);
    expect(buildScale9Mapper(data.commands, makeGrid(10, 10, 80, 80), 1, -1)).toBeNull();
  });

  it('returns null when commands have no drawable points', () => {
    const data = createShapeData();
    expect(buildScale9Mapper(data.commands, makeGrid(10, 10, 80, 80), 2, 2)).toBeNull();
  });

  it('maps corner coordinates unchanged (start region)', () => {
    const data = createShapeData();
    drawRect(data, 0, 0, 100, 100);
    const mapper = buildScale9Mapper(data.commands, makeGrid(10, 10, 80, 80), 2, 2)!;
